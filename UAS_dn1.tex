...
\item[(d)] Po (a) vemo, da je $G$ mrežno urejena. Po točkah (b) in (c) vemo, da so vse vrednosti elementov iz $G$ ravno $\Lambda=\{V_{E_{11}}, V_{E_{12}}, V_{E_{21}}, V_{E_{22}}\}$. Množico $\Lambda$ uredimo z relacijo nasprotno inkluziji, kar pomeni, da je za $A,B\in \Lambda$ $A\le B$ natanko tedaj, ko je $B\subseteq A$. Grupo $G$ potem vložimo v grupo $\prod_{\lambda \in \Lambda} \R_{\lambda} = \R_{V_{E_{11}}} \times \R_{V_{E_{12}}} \times\R_{V_{E_{21}}} \times \R_{V_{E_{22}}}$  z operacijo seštevanja. 

UREJENOST NA PRODUKTU 

Urejenost na produktu pa je definirana takole. Pogledamo na katerih mestih ima element neničelne komponente, potem pa med vsemi takimi indeksi iz $\Lambda$ izberemo tistega, ki je minimalne v urejenosti na $\Lambda$ in pogledamo, kakšnega predznaka je tista komponenta
.\end{enumerate}
