...
Pokažimo še obrat. Naj bo $r\in \R(X)$ in naj obstaja $\epsilon >0$, da je $r|_{(0,\epsilon)}\ge0$. Funkcijo $r$ lahko zapišemo kot $r(X) = X^m \tilde{r}(X)$, kjer je $\tilde{r}$ dobro definirana in neničelna v 0. Pokazati moramo še, da je strogo pozitivna v 0. Ker je $X^m>0$ na $(0,\infty)$, je tudi $\tilde{r} (X) = r(X) / X^m \ge 0$ na $(0,\epsilon)$. Zaradi zveznosti je potem $\tilde{r}(0) \ge 0$, po predpostavki pa je $\tilde{r}(0) \neq 0$. Funkcijo $r\in S$ smo zapisali kot $r(X) = X^m \tilde{r}(X)$, kjer je $m\in \Z$ in je $\tilde{r}$ dobro definirana in strogo pozitivna v 0. Sledi, da je $r\in Q$, torej $Q=S$.
